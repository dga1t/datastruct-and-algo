...
//Function to explore each vertex
void Explore(int x,vector<vector<int> >& adj, vector<int>& visited )
{
	//int x is the source vertex from which we will move to our destination y
	//visited array to save 1 if a vertex is visited or 0 if not
	visited[x] = 1;       //we start by the first vertex and giving it a true value that it's explored,
  ...
  //we ittereate about the row vector that exists in the adj 2D vector whose index is x
...
int main() {
  size_t n, m;
	...
	//input example - 4 2 these are 4 vertices and 2 edges
	//since this program is about undirected graph therefor each edge can go in any way for exxample :
	...
	for (int i = 0; i < m; i++) {
		int x, y;
		cin >> x >> y;
		//if the input is 1 2 we push vertix 2 into the array of vertix 1
		//and we also push vertix 1 into the array of vertix 2 so that they can be reachable from each other
