...
// We can sort them in order, pre order or post oder.

// By depth we mean that we completely traverse one sub-tree before moving on to a sibling sub-tree.
// That is opposite to the breadth-first method that we traverse all the nodes in each level then we move to the next level.
...
class TreeOrders {
  ...
  vector <int> right;   //vextor to store the right nodes
...
public:
  ...
  //return the key of the active node when we are moving to it's left subtree
  void pre_order(vector <int>& result, int node) 
  {
    ...
    //We push the node's key if we passed by it no matter what, so we first print the parent node of the whole tree
    //and since we move left first we will print the 2 the 1 and we go right to print 3 and we move up again but we find that all of them were pushed,
    //then we move right.
  }
  ...
  vector <int> IN() {
    vector<int> result;
    in_order(result, 0);    //the starting node is zero - we start from the parent node with index 0
    return result;
  }

  vector <int> PRE() {
    vector<int> result;    
    pre_order(result, 0);   //the starting node is zero - we start from the parent node with index 0
    return result;
  }

  vector <int> POST() {
    vector<int> result;
    post_order(result, 0);  //the starting node is zero - we start from the parent node with index 0
    return result;
  }
};
