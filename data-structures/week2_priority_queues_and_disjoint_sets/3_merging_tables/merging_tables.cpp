...
struct DisjointSetsElement {
	...
	//strcture various information about something by declaring different variables in it for the element instead of defining these variables multiple times for different elements
	int size, parent, rank;			//declaring the size of the table, its parent and its rank - by rank here we mean its height
...
struct DisjointSets {
	...
	//By path compression we mean that going from the selected node if it's not the table's parent we move up until we find the table's parent
	...
	int getParent(int table) {
		if (table != sets[table].parent) {
			sets[table].parent = getParent(sets[table].parent);		//we call the function recursively to find the parent of this node
		}
	...
	void Union(int i, int j) {		//int i and j are two tables to be merged
		...
		if (i_id != j_id) {		//if the two tables doesn't have the same parent
			...
			if (sets[j_id].rank > sets[i_id].rank) {		//if the height (rank) of the second tree is larger than the first
				...
				//and that's because by that we decrease the depth(rank) or height of the resulting tree or table to reduce the run time of merging as it is O(Tree Height)
...
int main() {
	...
	vector<int> destinations(m), sources(m);		//declaring two vectors to store the destination and source queries in respectively
